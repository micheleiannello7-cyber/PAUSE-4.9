...
- Tile categorie: altezza fissa 104 (tutte uguali), contenuto centrato (orb → nome → conteggio), chevron rimosso, badge check in alto a destra da selezionata; vetro più smerigliato (`glassFrost`) e blur ridotto (14). Testing agent iteration_22: PASS.
- Sfondo uniforme (foto notte con tono navy equalizzato, bokeh distribuito, scrim uniforme 0.40) e card categorie quasi trasparenti (glassTint 0.045, frost 0.06, tinta accento interna 0.07): vetro vero con bordo/glow colorato.
- Vetro limpido: rimossi sheen/frost/blur dalle card (GlassPressable), su web niente BlurView (aggiungeva velo grigio); unico riempimento rgba(120,175,255,0.03) + linea riflesso 1px + bordo/glow colore icona 35%.
- Icone categorie 3D "vetro liquido" (stile icona iOS): `GradientOrb` ora è un squircle a gradiente pieno e saturo (non più tinta traslucida su vetro), con riflesso lucido concentrato in alto-sinistra, ombra interna in basso per il volume, bordo luminoso e linea di riflesso sul bordo superiore; glow colorato più marcato. Risolve il feedback "icone troppo piatte" rispetto al riferimento.
