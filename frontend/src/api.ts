// PAUSE — API client. Uses EXPO_PUBLIC_BACKEND_URL from env.
...
export function categoryIllustrationUrl(cat: Category): string | null {
  if (!cat.illustration_generated) return null;
  return `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/category-media/${cat.id}`;
}
