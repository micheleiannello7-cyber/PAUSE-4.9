        ...
        <SectionList
          ...
          renderSectionHeader={({ section }) => (
            <View style={styles.sectionHead} testID={`saved-section-${section.cat.id}`}>
              <CategoryOrb id={section.cat.id} size={28} radiusOverride={9} />
...
function CatChip({
  label, catId, color, active, onPress, testID,
}: { label: string; catId?: string; color: string; active: boolean; onPress: () => void; testID: string }) {
  const styles = useStyles();
  const { colors } = useTheme();
  return (
    <Pressable
  ...
  return (
    <Pressable onPress={onPress} testID={`bookmark-${story.id}`} style={({ pressed }) => [styles.card, { opacity: pressed ? 0.92 : 1 }]}>
      {isLesson(story) && !story.hero_image_generated ? (
        <LessonCover color={story.category_color} icon={story.category_icon} categoryId={story.category_id} iconSize={28} showBadge={false} style={styles.thumb} />
      ) : (
